...
if (!empty($_SESSION) AND $_SESSION['userlevel'] == USER_ADMIN) {
	echo "<h2>Assigner des modérateurs</h2>";
	$query_subject= "SELECT * FROM subject WHERE moderatorID IS NULL AND visibility >= 1";
	...
	if ($result_subject= mysqli_query($db_connect, $query_subject)) {
		...
		while($row_subject = mysqli_fetch_array($result_subject)){
			...
			echo'
			<tr>
				<td> <a href="index.php?rq=modo_choose&subject_id='.$subjectID.'">'.$row_subject['title'].'</a> </td>
